...
import kotlinx.android.synthetic.main.column_task.view.*
import uz.myapigrid.taskmy.adapter.BoardAdapter
...
class BoardFragment : Fragment(),
    ...
    private val viewModel: BoardVM by viewModels()
    ...
    private val observerIconsStatus = Observer<Status> {
        when (it) {
            is Status.LOADING -> {
    ...
    private val observerUpdateIconStatus = Observer<Status> {
        when (it) {
            is Status.LOADING -> {
                progressbar.isVisible = true
            }
    ...
    private fun addColumn(iconList: ArrayList<IconModel>) {
        ...
        val footer = View.inflate(activity, R.layout.column_task, null)
